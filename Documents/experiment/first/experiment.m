polhemus=load('f_Polh_tf.txt');     #Load data from Polhemus  (tf2_msgs/TFMessage)
ts=load('f_ts_pose.txt');           #Load data from total station (geometry_msgs/PoseStamped= std_msgs/Header+geometry_msgs/Pose)
                                    #geometry_msgs/Pose= geometry_msgs/Point + geometry_msgs/Quaternion 
tstimedata=load('f_ts_data.txt');        #Load time data. It needs to be loaded from ts_data                      
                          
                       
 NUMDATAPOLH=3904;             #Number of data from the two different sensors.
 NUMDATATS=310;                                           
...
subplot(3,3,1); plot(time_polh,trans_x_polh); title('trans_x vs time'); hold on;
plot(time_ts,trans_x_ts); hold off;
subplot(3,3,2); plot(time_ts,errorx); title('error');
subplot(3,3,3); plot(trans_x_ts, errorx);
